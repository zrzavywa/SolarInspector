...
def parse_runtime_args(
    ...
    """Parse the existing application command-line options."""
    parser = argparse.ArgumentParser(
        description=f"{PRODUCT_NAME} {version}",
    )
    parser.add_argument(
        "--host",
...
def run_application(
    ...
    *,
    ...
    """Run the existing application startup sequence."""
    if args.demo_data:
        generate_demo_data(
            days=max(
                1,
    ...
    log_message(f"{PRODUCT_NAME} {version} läuft unter {url} (Bind: {host}:{port}).")
