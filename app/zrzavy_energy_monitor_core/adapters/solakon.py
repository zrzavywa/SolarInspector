...
class ModbusTcpConnection:
    ...
    def read_holding_registers(self, address: int, count: int) -> list[int]:
        ...
        with self._lock:
            ...
            request = (
                struct.pack(">HHHB", transaction, 0, len(pdu) + 1, self.unit_id) + pdu
            )
            try:
                self._socket.sendall(request)
                header = self._recv_exact(self._socket, 7)
                rx_transaction, protocol, length, rx_unit = struct.unpack(
                    ">HHHB", header
                )
                ...
                raise ModbusError(
                    f"Modbus-Kommunikation fehlgeschlagen: {exc}"
                ) from exc
...
class SolakonOneReader:
    ...
    def _create_connection(
        ...
        """Create the Modbus connection used by the reader."""
    ...
    def read(self, config: dict[str, Any]) -> SolakonOneReading:
        ...
        if not host:
            raise ValueError(
                "Keine IP-Adresse oder kein Hostname für Solakon ONE konfiguriert."
            )
        ...
        with self._create_connection(
            ...
            for start, count in self.BLOCKS:
                try:
                    ...
                    values.update(
                        {start + index: value for index, value in enumerate(registers)}
                    )
        ...
        if not values:
            raise ModbusError(
                "Der Solakon ONE antwortet, aber es konnten keine Register gelesen werden."
            )
        if 30000 not in values and 39118 not in values and 39134 not in values:
            raise ModbusError(
                "Keine erkennbaren Solakon-ONE-Register in der Antwort gefunden."
            )
        ...
        reading = SolakonOneReading(
            model_name=self._string(values, 30000, 16)
            or self._string(values, 39002, 16),
            serial_number=self._string(values, 30016, 16)
            or self._string(values, 39018, 16),
            status=status,
    ...
    @classmethod
    def _scaled_i16(
        cls, values: dict[int, int], address: int, scale: float
    ) -> Optional[float]:
