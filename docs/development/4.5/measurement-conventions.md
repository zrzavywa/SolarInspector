...
| --- | --- | --- |
| `GRID_METER` | Signed measurement at the selected public-grid connection point | Existing `house_meter` Shelly 3EM/Pro 3EM compatibility source; Solakon connected meter; future official grid meter |
| `HOUSE_METER` | Independent household-consumption or phase measurement | Reserved; no Phase 04 production adapter emits this role |
...
- `MEASURED`: reserved for a value explicitly classified as a physical
  measurement by a later source-specific decision.
- `REPORTED`: values returned by the current Shelly and Solakon production
  devices.
- `CALCULATED`: values generated by the current simulation adapters.
...
## Implemented acquisition path

Phase 04 production acquisition uses:

```text
Shelly or Solakon device
    -> existing device-specific parser
    -> normalized DeviceSnapshot
    -> explicit temporary legacy mapping
    -> unchanged collector calculations and persistence
```

The Shelly legacy reader retains its historical numeric `power_w=0.0` fallback
for direct compatibility. It also records whether a real power field was
available. The normalized adapter omits a missing required power measurement
and returns a `DEGRADED` partial snapshot, so a missing field is no longer
misrepresented as a valid normalized zero.

The collector currently supplies stable compatibility identifiers
`house_meter`, `solakon_meter`, and `solakon_one`. Moving those identifiers into
a fully migrated source registry is a later configuration task.
