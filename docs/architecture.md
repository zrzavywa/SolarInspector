...
Dieses Dokument beschreibt den implementierten Stand von **Zrzavy Energy Monitor 4.5.2**. Repositorypfade ohne Präfix sind relativ zur Repositorywurzel. Pfade unter `/opt`, `/etc`, `/var/lib`, `/var/cache` und `/var/log` sind Deploymentpfade, keine Repositorypfade.
